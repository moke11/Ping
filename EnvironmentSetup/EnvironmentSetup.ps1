...
function _LaunchCloudFormationStack([string]$instanceType, [string]$keyPair, [bool]$openRDP)
{
    ...
    $stackId = New-CFNStack -StackName "Ping" -Capability "CAPABILITY_IAM" -Parameter $parameters -TemplateBody $templateBody
    $stackId
}

function _SetupCodeDeployResources([string]$serviceRole)
{    
    _SetupCodeDeployApplication "Ping" $serviceRole
}
